...
import { montant, nombre, normaliser } from '@gmao/partage';
import type { BonCommande } from '@gmao/partage';
          ...
          detail && !['receptionnee', 'annulee'].includes(detail.statut) ? (
            <>
              {detail.statut === 'a_valider' && (
                <Bouton onClick={() => void commander(() => api.validerCommande(detail.id))}>
                  <CheckCheck className="size-4" /> Valider
                </Bouton>
              )}
              <Bouton onClick={() => void commander(() => api.receptionnerCommande(detail.id, true))}>
                Réception partielle
              </Bouton>
              <Bouton variante="primaire" onClick={() => void commander(() => api.receptionnerCommande(detail.id))}>
