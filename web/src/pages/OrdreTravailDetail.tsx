...
import { coutMainOeuvre, coutPieces, coutTotal, estEnRetard, estOuvert, estTermine, margeSLAHeures, tempsPasseMin } from '@gmao/partage';
import { dateCourte, dateHeure, echeanceRelative } from '@gmao/partage';
import { duree, montant, nombre } from '@gmao/partage';
import { CAUSE_PANNE, PRIORITE, STATUT_OT } from '@gmao/partage';
import type { CausePanne, OrdreTravail, ReleveMesure, StatutOT } from '@gmao/partage';
...
export function PageOrdreTravailDetail() {
  ...
  /** Renseigne un champ de terrain de l'OT (mesures, sécurité, coût externe). */
  const renseigner = (d: Parameters<typeof api.renseignerOT>[1]) =>
    void commander(() => api.renseignerOT(ot.id, d));
  ...
  return (
    <>
        ...
        actions={
          <>
            ...
            {ouvert && ot.statut !== 'en_cours' && (
              <Bouton
                variante="secondaire"
                onClick={() => void commander(() => api.changerStatutOT(ot.id, 'en_cours'))}
              >
            ...
            {ot.statut === 'realise' && (
              <Bouton
                variante="primaire"
                onClick={() => void commander(() => api.changerStatutOT(ot.id, 'cloture'))}
              >
      ...
      <CorpsPage>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="space-y-4 lg:col-span-2">
            ...
            {gamme && <CarteOperations ot={ot} gammeId={gamme.id} onRenseigner={renseigner} />}
                    ...
                    <li key={i} className="flex items-center gap-3 px-4 py-2.5">
                      ...
                      {!estTermine(ot) && (
                        <BoutonIcone
                          ...
                          onClick={() => void commander(() => api.retirerTemps(ot.id, i))}
                        >
                      ...
                      <li key={i} className="flex items-center gap-3 px-4 py-2.5">
                        ...
                        {!p.sortieValidee && (
                          <BoutonIcone
                            ...
                            onClick={() => void commander(() => api.retirerPiece(ot.id, i))}
                          >
            ...
            >
              ...
              <div className="mt-3 flex gap-2">
                ...
                <Bouton
                  ...
                  onClick={async () => {
                    await commander(() => api.commenterOT(ot.id, commentaire));
                    setCommentaire('');
                  }}
                >
                  Publier
              ...
              <div className="space-y-2.5">
                {(
                  ...
                  <CaseACocher
                    ...
                    onChange={(e) => renseigner({ securite: { ...ot.securite, [cle]: e.target.checked } })}
                  />
                ))}
              </div>
            </Carte>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {!estTermine(ot) && ot.execution === 'prestataire' && (
                          <Saisie
                            ...
                            placeholder="montant"
                            onBlur={(e) => renseigner({ coutPrestataire: Number(e.target.value) || 0 })}
                          />
                        )}
                      </div>
                    ),
                  ...
                  {TRANSITIONS[ot.statut].map((s) => (
                    <Bouton
                      ...
                      onClick={() => {
                        ...
                        else void commander(() => api.changerStatutOT(ot.id, s));
                      }}
                    >
                      {STATUT_OT[s].libelle}
                    </Bouton>
      ...
      <ModaleTemps
        ...
        onValider={async (technicienId, dureeMin, texte) => {
          await commander(() => api.saisirTemps(ot.id, { technicienId, dureeMin, commentaire: texte }));
          setModale(null);
        }}
      />
      ...
      <ModalePiece
        ...
        onValider={async (articleId, quantite) => {
          await commander(() => api.ajouterPiece(ot.id, articleId, quantite));
          setModale(null);
        }}
      />
      ...
      <ModaleAffectation
        ...
        onValider={async (technicienId, date) => {
          await commander(() => api.affecterOT(ot.id, technicienId || undefined, date));
          setModale(null);
        }}
      />
      ...
      <ModaleCloture
        ...
        onValider={async (donnees) => {
          await commander(() =>
            api.cloturerOT(ot.id, {
              diagnostic: donnees.diagnostic || undefined,
              causePanne: donnees.causePanne,
              actionsRealisees: donnees.actionsRealisees,
              arretEquipementMin: donnees.arretMin,
              conformite: donnees.conformite ?? 'conforme',
              cloturer: donnees.cloturer,
            }),
          );
          setModale(null);
        }}
...
/** Remplace la mesure d'une opération, ou l'ajoute si elle n'existe pas encore. */
function remplacerMesure(
  mesures: ReleveMesure[],
  operationId: string,
  modification: Partial<ReleveMesure>,
): ReleveMesure[] {
  const existe = mesures.some((m) => m.operationId === operationId);
  if (!existe) return [...mesures, { operationId, conforme: false, ...modification }];
  return mesures.map((m) => (m.operationId === operationId ? { ...m, ...modification } : m));
}
...
function CarteOperations({
  ...
  ot: OrdreTravail;
  gammeId: string;
  onRenseigner: (d: { mesures: ReleveMesure[] }) => void;
}) {
  const { index } = useGMAO();
              ...
              <tr key={op.id} className={horsTolerance ? 'bg-rose-50/50' : ''}>
                  ...
                  {op.valeurAttendue !== undefined ? (
                    <Saisie
                      ...
                      onBlur={(e) => {
                        ...
                        onRenseigner({ mesures: remplacerMesure(ot.mesures, op.id, { valeur: v, conforme }) });
                      }}
                    />
                  ) : (
                ...
                <td className="px-3 py-2 text-center">
                  <input
                    ...
                    onChange={(e) =>
                      onRenseigner({
                        mesures: remplacerMesure(ot.mesures, op.id, { conforme: e.target.checked }),
                      })
                    }
