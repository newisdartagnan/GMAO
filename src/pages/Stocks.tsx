...
import { genererReappro, mouvementerStock } from '@/data/actions';
import { dateCourte, joursRestants } from '@/lib/dates';
import { montant, nombre, normaliser, pluriel } from '@/lib/format';
import { TYPE_MOUVEMENT } from '@/types/labels';
import type { Article, MouvementStock } from '@/types/domain';
    ...
    <>
      ...
      {mouvement && (
        <ModaleMouvement
          ...
          onValider={(type, quantite, motif) => {
            muter(
              `Mouvement de stock ${type} : ${mouvement.designation}`,
              (b) => mouvementerStock(b, mouvement.id, type, quantite, motif, utilisateur.id),
              { action: 'modification', entiteType: 'article', entiteId: mouvement.id },
            );
            setMouvement(null);
          }}
        />
          ...
          <>
            ...
            <Bouton
              ...
              onClick={() => {
                let nb = 0;
                muter(
                  'Génération des commandes de réapprovisionnement',
                  (b) => {
                    nb = genererReappro(b, utilisateur.id).length;
                  },
                  { action: 'generation', entiteType: 'bon_commande', entiteId: '-' },
                );
                setReappro(false);
                setResultatReappro(nb);
              }}
            >
              Générer
            </Bouton>
