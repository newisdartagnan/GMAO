...
import { enregistrerVisiteControle, leverReserve } from '@/data/actions';
import { conformite, equipementsDuControle } from '@/lib/kpi';
import { prochaineEcheanceControle } from '@/lib/echeances';
import { dateCourte, joursRestants, libellePeriodicite } from '@/lib/dates';
import { nombre } from '@/lib/format';
import { REFERENTIEL } from '@/types/labels';
import type { ControleReglementaire, ID, Reserve } from '@/types/domain';
    ...
    <>
                  ...
                  {r.reserve.levee ? (
                    ...
                    <>
                      ...
                      <Bouton
                        taille="sm"
                        onClick={() =>
                          muter(`Levée de réserve : ${r.reserve.libelle}`, (b) => leverReserve(b, r.reserve.id), {
                            action: 'validation',
                            entiteType: 'reserve',
                            entiteId: r.reserve.id,
                          })
                        }
                      >
                        Lever
      ...
      {saisie && (
        <ModaleVisite
          ...
          onValider={(donnees) => {
            muter(
              `Visite de contrôle enregistrée`,
              (b) => enregistrerVisiteControle(b, { controleId: saisie.controleId, equipementId: saisie.equipementId, ...donnees }),
              { action: 'creation', entiteType: 'visite_controle', entiteId: saisie.equipementId },
            );
            setSaisie(null);
          }}
        />
