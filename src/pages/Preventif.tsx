...
import { genererOTPreventifs } from '@/data/actions';
import { echeancierPreventif, equipementsDeLaGamme } from '@/lib/kpi';
import type { EcheancePreventive } from '@/lib/kpi';
import { seuilCompteurSuivant } from '@/lib/echeances';
import { dateCourte, libellePeriodicite } from '@/lib/dates';
import { duree, nombre, pluriel } from '@/lib/format';
import type { GammeMaintenance } from '@/types/domain';
    ...
    <>
      ...
      <ModaleGeneration
        ...
        onGenerer={(jours) => {
          let nb = 0;
          muter(
            `Génération des OT préventifs à ${jours} jours`,
            (b) => {
              nb = genererOTPreventifs(b, jours, utilisateur.id).length;
            },
            { action: 'generation', entiteType: 'gamme', entiteId: '-' },
          );
          setGeneration(false);
          setResultat(nb);
        }}
