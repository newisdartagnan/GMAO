...
import { estOuvert, tauxVetuste, valeurNetteComptable } from '@/lib/kpi';
import { montant, normaliser, nombre } from '@/lib/format';
import { joursRestants } from '@/lib/dates';
import { CRITICITE, DOMAINE, STATUT_EQUIPEMENT } from '@/types/labels';
import type { Equipement, NiveauCriticite } from '@/types/domain';
import { uid } from '@/lib/id';
import { iso, aujourdHui } from '@/lib/dates';

export function PageEquipements() {
  const { base, index, muter } = useGMAO();
    ...
    <>
      ...
      <ModaleCreationEquipement
        ...
        onCreer={(brouillon) => {
          ...
          muter(
            `Création de l’équipement ${brouillon.code}`,
            (b) => {
              const local = b.locaux.find((l) => l.id === brouillon.localId)!;
              b.equipements.push({
                id: nouvelId,
                code: brouillon.code,
                designation: brouillon.designation,
                familleId: brouillon.familleId,
                domaine: b.familles.find((f) => f.id === brouillon.familleId)?.domaine ?? 'biomedical',
                marque: brouillon.marque,
                modele: brouillon.modele,
                numeroSerie: brouillon.numeroSerie,
                classeDM: brouillon.classeDM,
                marquageCE: brouillon.classeDM !== 'hors_DM',
                soumisVigilance: ['IIb', 'III'].includes(brouillon.classeDM),
                siteId: local.siteId,
                batimentId: local.batimentId,
                localId: local.id,
                serviceId: local.serviceId,
                statut: 'en_service',
                criticite: brouillon.criticite,
                dateAcquisition: brouillon.dateAcquisition,
                dateMiseEnService: brouillon.dateMiseEnService,
                valeurAchat: brouillon.valeurAchat,
                dureeAmortissementAns: b.familles.find((f) => f.id === brouillon.familleId)?.dureeVieAns ?? 8,
                finGarantie: brouillon.finGarantie || undefined,
                fournisseurId: brouillon.fournisseurId || undefined,
                fabricantId: brouillon.fournisseurId || undefined,
                compteurs: [],
                risqueInfectieux: brouillon.risqueInfectieux,
                sourceRadioactive: false,
                gazMedicaux: false,
                alimentationSecourue: brouillon.criticite <= 2,
                objectifDisponibilite: brouillon.criticite === 1 ? 99 : 95,
                qrToken: `GMAO:${brouillon.code}`,
                actif: true,
                notes: brouillon.notes || undefined,
              });
            },
            { action: 'creation', entiteType: 'equipement', entiteId: nouvelId },
          );
          setCreation(false);
