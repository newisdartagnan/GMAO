...
import { creerVigilance, marquerEquipementTraite } from '@/data/actions';
import { aujourdHui, dateCourte, iso, joursRestants } from '@/lib/dates';
import { nombre, pourcent } from '@/lib/format';
import type { RappelFabricant, Vigilance } from '@/types/domain';
        ...
        pied={
          detail && detail.statut !== 'clos' ? (
            <Bouton
              ...
              onClick={() =>
                muter(
                  `Clôture de la vigilance ${detail.numero}`,
                  (b) => {
                    const v = b.vigilances.find((x) => x.id === detail.id);
                    if (v) {
                      v.statut = 'clos';
                      v.dateCloture = iso(aujourdHui());
                    }
                  },
                  { action: 'cloture', entiteType: 'vigilance', entiteId: detail.id },
                )
              }
            >
              Clôturer le dossier
                ...
                return (
                  <li key={id} className="flex items-center gap-3 px-3 py-2">
                    <input
                      ...
                      onChange={() =>
                        muter(
                          `Rappel ${rappel.reference} traité sur ${eq?.code}`,
                          (b) => {
                            marquerEquipementTraite(b, rappel.id, id);
                            const maj = b.rappels.find((x) => x.id === rappel.id);
                            if (maj) setRappel({ ...maj });
                          },
                          { action: 'modification', entiteType: 'rappel', entiteId: rappel.id },
                        )
                      }
...
function ModaleDeclaration({ ouverte, onFermer }: { ouverte: boolean; onFermer: () => void }) {
  const { base, muter, utilisateur } = useGMAO();
      ...
      pied={
        <>
          ...
          <Bouton
            ...
            onClick={() => {
              muter(
                `Déclaration de matériovigilance sur ${base.equipements.find((e) => e.id === equipementId)?.code}`,
                (b) =>
                  creerVigilance(b, {
                    equipementId,
                    dateEvenement: iso(aujourdHui()),
                    dateDeclaration: declarer ? iso(aujourdHui()) : undefined,
                    description,
                    gravite,
                    ...
                    declarantId: utilisateur.id,
                  }),
                { action: 'creation', entiteType: 'vigilance', entiteId: equipementId },
              );
              onFermer();
            }}
          >
