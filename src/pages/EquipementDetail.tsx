...
import { alertesDeLEquipement } from '@/lib/alertes';
import {
  coutTotal,
  estOuvert,
  fiabilite,
...
import { prochaineEcheanceControle, prochaineEcheanceGamme, seuilCompteurSuivant } from '@/lib/echeances';
import { aujourdHui, dateCourte, dateHeure, echeanceRelative, iso, joursRestants, libellePeriodicite } from '@/lib/dates';
import { duree, heures, montant, nombre, pourcent } from '@/lib/format';
import { CAUSE_PANNE, CRITICITE, DOMAINE, REFERENTIEL, STATUT_EQUIPEMENT } from '@/types/labels';
import type { OrdreTravail, StatutEquipement } from '@/types/domain';
import { changerStatutEquipement, creerOT, deplacerEquipement, releverCompteur } from '@/data/actions';

export function PageEquipementDetail() {
  const { id } = useParams();
  const { base, index, alertes, muter, utilisateur } = useGMAO();
  ...
  return (
    <>
      ...
      <ModaleDeplacement
        ...
        onFermer={() => setModale(null)}
        onValider={(localId, motif) => {
          muter(
            `Déplacement de ${equipement.code}`,
            (b) => deplacerEquipement(b, equipement.id, localId, motif, utilisateur.id),
            { action: 'modification', entiteType: 'equipement', entiteId: equipement.id },
          );
          setModale(null);
        }}
      />
      ...
      {equipement.compteurs.length > 0 && (
        <ModaleCompteur
          ...
          onValider={(valeur) => {
            muter(
              `Relevé de compteur sur ${equipement.code} : ${valeur} ${equipement.compteurs[0].unite}`,
              (b) => releverCompteur(b, equipement.id, valeur),
              { action: 'modification', entiteType: 'equipement', entiteId: equipement.id },
            );
            setModale(null);
          }}
      ...
      <ModaleStatut
        ...
        onValider={(statut) => {
          muter(
            `Statut de ${equipement.code} : ${STATUT_EQUIPEMENT[statut].libelle}`,
            (b) => changerStatutEquipement(b, equipement.id, statut),
            { action: 'modification', entiteType: 'equipement', entiteId: equipement.id },
          );
          setModale(null);
        }}
        ...
        <>
          ...
          <Bouton
            ...
            onClick={() => {
              let nouveauId = '';
              muter(
                `Création d’un OT ${f.type} : ${f.objet}`,
                (b) => {
                  const ot = creerOT(
                    b,
                    {
                ...
                { action: 'creation', entiteType: 'ordre_travail', entiteId: equipementId ?? '-' },
              );
              onFermer();
              if (nouveauId) naviguer(`/ordres-travail/${nouveauId}`);
            }}
          >
            Créer l’OT
          </Bouton>
