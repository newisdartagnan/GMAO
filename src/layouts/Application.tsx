...
import { Bell, ChevronDown, Menu, ScanLine, Search, X } from 'lucide-react';
import { NAVIGATION } from './navigation';
import { useGMAO } from '@/data/store';
import { compterAlertes } from '@/lib/alertes';
import { estEnRetard, estOuvert } from '@/lib/kpi';
import { Avatar } from '@/components/shared/Liens';
import { ROLE } from '@/types/labels';
import { RechercheGlobale } from '@/components/shared/RechercheGlobale';

export function Application() {
  const { base, alertes, utilisateur, changerUtilisateur } = useGMAO();
      ...
      >
        ...
        <div className="shrink-0 border-t border-slate-200 p-3">
          <div className="relative">
            ...
            {selecteurOuvert && (
              <div className="absolute bottom-full left-0 mb-1 max-h-80 w-full overflow-y-auto rounded-lg border border-slate-200 bg-white py-1 shadow-lg">
                <p className="px-3 py-1.5 text-[11px] font-semibold tracking-wide text-slate-400 uppercase">
                  Changer de profil
                </p>
                {base.utilisateurs
                  .filter((u) => u.actif && u.role !== 'demandeur')
                  .map((u) => (
                    <button
                      key={u.id}
                      type="button"
                      ...
                      className={`flex w-full items-center gap-2 px-3 py-1.5 text-left text-sm hover:bg-slate-50 ${
                        u.id === utilisateur.id ? 'bg-marque-50 text-marque-800' : 'text-slate-700'
                      }`}
                    >
                      <Avatar nom={u.nom} prenom={u.prenom} />
                      <span className="min-w-0 flex-1 truncate">
                        {u.prenom} {u.nom}
                        <span className="block truncate text-xs text-slate-500">{ROLE[u.role]}</span>
                      </span>
                    </button>
                  ))}
              </div>
