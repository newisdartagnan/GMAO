...
import { addDays, addMonths, aujourdHui, iso, isoHeure, toDate } from './dates';
...
export function construireBaseDemo(): BaseGMAO {
  // Les homonymes existent dans un établissement de cette taille : l'adresse
  // électronique doit rester unique, c'est elle qui sert d'identifiant de
  // connexion.
  const emailsAttribues = new Set<string>();
  ...
  const modelesInspection: ModeleInspection[] = CATALOGUE_INSPECTIONS.map((m, i) => ({
    id: `min_${String(i + 1).padStart(3, '0')}`,
    code: m.code,
    libelle: m.libelle,
    designations: m.designations,
    shifts: m.shifts,
    equipeId: equipes.find((e) => e.domaine === (m.code === 'INS-O2' || m.code === 'INS-FLU' ? 'fluides_medicaux' : 'technique_batiment'))?.id,
    dureeEstimeeMin: m.duree,
    consignesSecurite: m.consignes,
    points: m.points.map((pt, k) => ({ ...pt, id: `pci_${i + 1}_${k + 1}` })),
    actif: true,
  }));

  const rondes: RondeInspection[] = [];
  const relevesInspection: ReleveInspection[] = [];
  const baseIntermediaire = { equipements, modelesInspection, sites, rondes, relevesInspection } as unknown as BaseGMAO;

  const JOURS_RONDES = 14;
  const agentsDeGarde = utilisateurs.filter(
    (u) => u.role === 'technicien' && ['eq_tec', 'eq_flu'].includes(u.equipeId ?? ''),
  );
  let rondeSeq = 1;

  for (let j = JOURS_RONDES; j >= 0; j -= 1) {
    const date = jourRelatif(-j);
    for (const shift of ['matin', 'soir'] as Shift[]) {
      const perimetre = perimetreRonde(baseIntermediaire, shift, 'site_hgr');
      if (!perimetre.length) continue;

      const heure = HORAIRES_SHIFT[shift].heure;
      // La ronde en cours de créneau reste ouverte ; une ronde sur vingt est
      // manquée, ce qui est l'ordre de grandeur observé sur le terrain.
      const passee = j > 0 || AUJ.getHours() > heure + 2;
      const manquee = passee && chance(0.05);
      const agent = choix(agentsDeGarde.length ? agentsDeGarde : techniciens);

      const ronde: RondeInspection = {
        id: `rnd_${String(rondeSeq).padStart(4, '0')}`,
        numero: `RI-${AUJ.getFullYear()}-${String(rondeSeq).padStart(5, '0')}`,
        date,
        shift,
        siteId: 'site_hgr',
        agentId: manquee ? undefined : agent.id,
        statut: manquee ? 'manquee' : passee ? 'terminee' : 'planifiee',
        heureDebut: manquee || !passee ? undefined : heureRelative(-j, heure),
        heureFin: manquee || !passee ? undefined : heureRelative(-j, heure + 1),
        equipementsAttendus: perimetre,
        signature: manquee || !passee ? undefined : `${agent.prenom} ${agent.nom}`,
        observations: !manquee && passee && chance(0.15)
          ? choix([
              'Coupure réseau de 22 h 10 à 23 h 40, groupe électrogène passé en charge sans incident.',
              'Rien à signaler sur l’ensemble du parcours.',
              'Local technique du bâtiment E à faire nettoyer, accès encombré.',
              'Réserve de gasoil à recompléter avant le week-end.',
            ])
          : undefined,
      };
      rondes.push(ronde);
      ...
      if (manquee || !passee) continue;

      // Sur une ronde réalisée, un ou deux équipements peuvent rester
      // inaccessibles : la ronde est alors incomplète, pas fausse.
      const aRelever = chance(0.12) ? echantillon(perimetre, perimetre.length - ent(1, 2)) : perimetre;
      for (const equipementId of aRelever) {
        const eq = equipements.find((e) => e.id === equipementId)!;
        const modele = modelesInspection.find((m) => m.shifts.includes(shift) && m.designations.includes(eq.designation));
        if (!modele) continue;

        const valeurs: ValeurReleve[] = modele.points.map((pt) => tirerValeur(pt, eq));
        const ecarts = valeurs.filter((v) => !v.conforme);
        const bloquant = ecarts.some((v) => modele.points.find((pt) => pt.id === v.pointId)?.bloquant);

        relevesInspection.push({
          id: `rli_${String(relevesInspection.length + 1).padStart(5, '0')}`,
          rondeId: ronde.id,
          equipementId,
          modeleId: modele.id,
          heure: heureRelative(-j, heure),
          agentId: agent.id,
          valeurs,
          etat: ecarts.length === 0 ? 'conforme' : bloquant ? 'anomalie_majeure' : 'anomalie_mineure',
          observation: ecarts.length && chance(0.4)
            ? choix([
                'Signalé au responsable technique à la relève.',
                'Contrôlé une seconde fois, valeur confirmée.',
                'Appoint réalisé sur place, à surveiller à la prochaine ronde.',
              ])
            : undefined,
        });
      }

      const av = avancementRonde(baseIntermediaire, ronde.id);
      if (av.restants.length) ronde.statut = 'incomplete';
      if (ronde.statut === 'terminee' && chance(0.7)) {
        ronde.viseParId = 'usr_003';
        ronde.dateVisa = heureRelative(-j, heure + 2);
      }
    }
  }
  ...
  const audit: EntreeAudit[] = [
    {
      ...
      details: `${equipements.length} équipements, ${ordresTravail.length} ordres de travail, ${articles.length} articles, ${rondes.length} rondes d’inspection.`,
    },
  ];
  ...
  return {
    ...
    modelesInspection, rondes, relevesInspection,
  };
  ...
  function adresseUnique(prenom: string, nom: string): string {
    const base = `${prenom.toLowerCase().normalize('NFD').replace(/[^a-z]/g, '')}.${nom
      .toLowerCase()
      .normalize('NFD')
      .replace(/[^a-z]/g, '')}`;
    let candidat = `${base}@hgr-kinshasa.cd`;
    let suffixe = 1;
    while (emailsAttribues.has(candidat)) {
      suffixe += 1;
      candidat = `${base}${suffixe}@hgr-kinshasa.cd`;
    }
    emailsAttribues.add(candidat);
    return candidat;
  }
  ...
  function utilisateur(
    ...
    return {
      ...
      email: adresseUnique(prenom, nom),
...
/**
 * Tire une valeur plausible pour un point de contrôle. Neuf relevés sur dix
 * tombent dans la plage attendue : c'est ce qui rend les écarts visibles.
 */
function tirerValeur(point: PointControle, equipement: Equipement): ValeurReleve {
  const brouillon: ValeurReleve = { pointId: point.id, conforme: true };

  switch (point.type) {
    case 'mesure': {
      const min = point.valeurMin ?? (point.valeurMax !== undefined ? point.valeurMax * 0.7 : 0);
      const max = point.valeurMax ?? (point.valeurMin !== undefined ? point.valeurMin * 1.35 + 1 : 100);
      const amplitude = Math.max(0.1, max - min);
      brouillon.valeurNum = chance(0.92)
        ? Number(dec(min + amplitude * 0.08, max - amplitude * 0.08, 2))
        : chance(0.5)
          ? Number(dec(min - amplitude * 0.25, min - amplitude * 0.02, 2))
          : Number(dec(max + amplitude * 0.02, max + amplitude * 0.25, 2));
      break;
    }
    case 'compteur': {
      const compteur = equipement.compteurs[0];
      brouillon.valeurNum = compteur ? Math.round(compteur.valeur - ent(0, 40)) : ent(1_000, 40_000);
      break;
    }
    case 'etat': {
      const options = point.options ?? ['Normal'];
      const conformes = point.optionsConformes ?? [options[0]];
      const autres = options.filter((o) => !conformes.includes(o));
      brouillon.valeurTexte = chance(0.93) || !autres.length ? choix(conformes) : choix(autres);
      break;
    }
    case 'booleen': {
      const attendu = point.reponseAttendue ?? true;
      brouillon.valeurBool = chance(0.95) ? attendu : !attendu;
      break;
    }
    case 'texte':
      brouillon.valeurTexte = chance(0.12)
        ? choix([
            'Relevé effectué avec le manomètre de contrôle.',
            'Local propre, accès dégagé.',
            'Ventilation à surveiller, chaleur inhabituelle.',
          ])
        : undefined;
      break;
  }

  brouillon.conforme = valeurConforme(point, brouillon);
  return brouillon;
}
